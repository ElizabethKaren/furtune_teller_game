...
const Nav = props => {
    ...
    if (['A','E','I','O','U','Y'].includes(props.sign[0])) return <div className='ui black label'><h3>An {props.sign} I see...</h3>{props.myMessages.length > 0 ? <Link to='/horiscope/inbox'><button className='ui small button'>{props.myMessages.length} Messages</button></Link> : null }</div>
    return (
